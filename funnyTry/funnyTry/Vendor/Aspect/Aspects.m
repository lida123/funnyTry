...
/// 被hook的方法信息，注意 originalInvocation.selector =  aspects__`originalSelectorName`, example: `aspects__viewWillLayoutSubviews`
/// originalInvocation.target = instance
...
@end
 ...
 /// 跟踪一个类的继承链中的hook状态：包括被hook的类，哪些SEL被hook了。
...
static id aspect_add(id self, SEL selector, AspectOptions options, id block, NSError **error) {
    ...
    aspect_performLocked(^{
        ...
        if (aspect_isSelectorAllowedAndTrack(self, selector, options, error)) {
            ...
            // B: 可以被 hook 的话，则生成一个 AspectIdentifier 并添加到 AspectsContainer 中， AspectsContainer 是由关联对象技术存储在 UIViewController 类对象中。
...
static BOOL aspect_remove(AspectIdentifier *aspect, NSError **error) {
    ...
    aspect_performLocked(^{
        ...
        if (self) {
            /// 从 aspectContainer 移除 AspectIdentifier
...
/// 在block中的参数(block,id<AspectInfo>,... )个数不能大于 selector(self,_cmd, ...) 方法的参数数量--------------------------------------
...
/// `viewDidLoad` -> `_objc_msgForward` 这样调用 `viewDidLoad` 就会直接执行 `_objc_msgForward`, 从而触发消息转发执行 `forwardInvocation:`,而 `forwardInvocation:`-> `__ASPECTS_ARE_BEING_CALLED__`。  所以最终调用 `viewDidLoad` 会执行 `__ASPECTS_ARE_BEING_CALLED__`.
...
static void aspect_cleanupHookedClassAndSelector(NSObject *self, SEL selector) {
    ...
    // 如果AspectsContainer 空了 去除AspectsContainer
    if (!container.hasAspects) {
        ...
        }else { // 还原 forwardInvocation: 方法实现
...
/// 一旦 hook 一个类，会生成一个 AspectTracker0，以 class 为key，AspectTracker0 为value存在静态字典中。
/// 同时，会获取这 个类的父类，同样生成一个 AspectTracker1，以 superClass 为key，AspectTracker1 为value存在静态字典中。
...
/// 拿到当前类的子类链中所有hook了指定方法（selectorName）的 AspectTracker’s
- (NSSet *)subclassTrackersHookingSelectorName:(NSString *)selectorName {
